...
% ------------------------------------------------------------
\input{output/tables/prob1_macros.tex}
\input{output/tables/prob_sw_ee_macros.tex}
...
* (ii) covariance ratio
sum logpgp95 avexpr logem4 if !missing(avexpr,logem4)
gen zy = (logem4 - r(mean_logem4)) * (logpgp95 - r(mean_logpgp95))  // illustrative
gen zx = (logem4 - r(mean_logem4)) * (avexpr   - r(mean_avexpr))
sum zy; scalar s_zy = r(mean)*(r(N)/(r(N)-1))
sum zx; scalar s_zx = r(mean)*(r(N)/(r(N)-1))
display s_zy/s_zx
...
\end{lstlisting}
The three avexpr coefficients are $\ajrcgbsecond$, $\ajrchbsecond$, and $\ajrcibsecond$, which round to the authors' $0.98$, $1.10$, and $0.98$. Column 7's continent dummies are $\ajrcgbsecondasia$ (Asia), $\ajrcgbsecondafr$ (Africa), $\ajrcgbsecondoth$ (other) vs.\ the authors' $-0.92$, $-0.46$, $-0.94$; column 8's are $\ajrchbsecondasia$, $\ajrchbsecondafr$, $\ajrchbsecondoth$ vs.\ $-1.10$, $-0.44$, $-0.99$; all match to two decimal places. The SE comparison is
\begin{center}
\footnotesize
...
\subsection*{Tables}
\singlespacing
\input{output/tables/prob1_table4.tex}
...
The \texttt{do\_files/} subfolder contains two Stata do-files: \texttt{ps7\_ajr.do} (Problem~1, parts a--g: Acemoglu--Johnson--Robinson 2001 Table~4 replication) and \texttt{ps7\_diff\_fig.do} (Problem~2, part d: the Card--Krueger DiD figure). Each writes to a matching \texttt{.log} file in the same directory. The \texttt{output/} subfolder contains the \LaTeX\ tables (\texttt{output/tables/}) and PDF figures (\texttt{output/figures/}) generated by those do-files and \verb|\input|ed into this document.
