...
from Yahoo Finance for Jan 1, 2021 – Jan 1, 2026. Computes percentage changes,
arranges as panel data, and exports to Stata .dta format.
...
# ============================================================
# pct_change() computes (P_t - P_{t-1}) / P_{t-1} * 100
pct_changes = prices.pct_change() * 100  # multiply by 100 for percent
pct_changes = pct_changes.dropna()  # drop the first row (NaN from differencing)

# Extract market returns
market_ret = pct_changes['SP500']

# ============================================================
# 4. Reshape to panel (long) format
# ============================================================
# Each row = (firm_id, date, pct_change_firm, pct_change_market)
panels = []
for i, ticker in enumerate(tickers, start=1):
    firm_data = pd.DataFrame({
        'firm_id': i,
        'ticker': ticker,
        'date': pct_changes.index,
        'pct_ret_firm': pct_changes[ticker].values,
        'pct_ret_market': market_ret.values
    })
...
panel_df.to_stata(output_path, write_index=False,
                  variable_labels={
                      ...
                      'pct_ret_firm': 'Percentage daily return of firm stock',
                      'pct_ret_market': 'Percentage daily return of S&P 500',
