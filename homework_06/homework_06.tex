...
\[
\hat{p} \;=\; \frac{1}{n}\sum_{i} \texttt{cardhldr}_i \;=\; \amexPhat,
\]
or about $78.3\%$, were approved as cardholders. The sample is therefore \emph{not} a random cross-section of American consumers: every row is a completed \textsc{AmEx} application. External validity is correspondingly limited. Two caveats follow.
\begin{itemize}
\item \textbf{Selection into the sample.} Consumers who never applied are absent; those who did apply are disproportionately drawn from higher-income, more financially sophisticated households with some desire (and perceived eligibility) for a premium charge card. Coefficients estimated on this frame therefore speak to the conditional approval process at American Express \emph{given applicant status}, not to the unconditional probability of being a cardholder in the U.S.\ population.
\item \textbf{Selection on unobservables.} Several covariates---notably \texttt{income} and the derogatory-credit counts---likely correlate with AmEx's unobserved internal credit score. Regression coefficients retain their interpretation as conditional means given observables, but any causal reading (``giving applicant $i$ a higher \texttt{income} would raise approval by $\hat{\beta}$'') requires much stronger assumptions than those licensed by the sampling design \citep[see][\S11.3]{StockWatson2020}.
\end{itemize}
The $78\%$ approval rate is high because applicants already self-select on observable eligibility before filing an application.
\end{solution}
...
p20 ($\$19$k) & $-0.1020$ & $-0.1274$ & $-0.1317$ & $-14.07\%$ & $-17.71\%$ & $-18.37\%$ \\
p50 ($\$26$k) & $-0.1020$ & $-0.1209$ & $-0.1222$ & $-13.55\%$ & $-15.99\%$ & $-16.13\%$ \\
p80 ($\$39$k) & $-0.1020$ & $-0.1068$ & $-0.1025$ & $-12.72\%$ & $-13.11\%$ & $-12.49\%$ \\[0.2em]
\multicolumn{7}{l}{\emph{$\Delta\mathit{ownrent}$}} \\
p20 ($\$19$k) & $+0.0501$ & $+0.0555$ & $+0.0549$ & $+6.91\%$  & $+7.71\%$  & $+7.67\%$  \\
p50 ($\$26$k) & $+0.0501$ & $+0.0511$ & $+0.0491$ & $+6.66\%$  & $+6.75\%$  & $+6.48\%$  \\
p80 ($\$39$k) & $+0.0501$ & $+0.0428$ & $+0.0388$ & $+6.25\%$  & $+5.25\%$  & $+4.73\%$  \\
\bottomrule
\end{tabular}
\end{center}
Two patterns deserve emphasis.
\begin{enumerate}
\item For the LPM the absolute marginal effect is identical across income levels, as it must be: linearity implies $\Delta\hat{P} = \hat{\beta}$ regardless of the remaining covariates.
\item For probit and logit the discrete changes \emph{shrink in magnitude} as income rises. At the $20$th percentile the conditional approval probability starts near $0.72$, so the latent index sits squarely where $\phi(\cdot)$ and $\Lambda'(\cdot)$ are largest; at the $80$th percentile $P_0$ has risen to $\approx 0.82$, pushing the index into the right-hand tail where the link-function slope is smaller. This is precisely the nonlinearity that LPM cannot capture.
\end{enumerate}
The qualitative conclusion survives: self-employment is uniformly bad news ($12$--$18\%$ lower approval), home ownership uniformly good news ($5$--$8\%$ higher), and the \emph{magnitude} of each dummy's effect depends on income for the nonlinear models but not for the LPM.
\end{solution}
...
\[
\text{OR}(\texttt{selfempl}) \;=\; e^{-0.5847} \;\approx\; 0.557,
\qquad
...
\begin{enumerate}
\item[(1)] The $\%\Delta$ row in the prescribed table asks for a proportional change, which is not invariant under switching the ``reference'' scenario ($P_0$ vs $P_1$). Does the problem set assume $\%\Delta = (P_1-P_0)/P_0$, or the symmetric $(P_1-P_0)/\tfrac{1}{2}(P_0+P_1)$?
\item[(2)] For the LPM, the ``log-odds'' is ill-defined whenever $\hat{P}$ exits $[0,1]$. How should we handle those observations in part~(e)? Should we trim to $\hat{P}\in(0,1)$, or report the log-odds only at the evaluation points for which both $P_0$ and $P_1$ remain interior?
\item[(3)] The interpretation ``odds ratio = $e^{\beta}$'' is exact in a logit; for probit the analogue is a unit shift in the latent-index $z$-score, which is not an odds ratio per se. Is there a standard way to express the probit marginal effect in odds-ratio language for comparison to logit?
\end{enumerate}
\end{reflection}
...
\begin{itemize}
\item \textbf{Derogatory events dominate.} In the LPM, each additional \emph{major} derogatory event cuts the approval probability by $11.7$ pp ($p<0.01$); each \emph{minor} event by $3.2$ pp. The logit and probit estimates imply very similar average marginal effects (see part~(j)).
\item \textbf{Income effect survives.} The coefficient on \texttt{inc} (per \$1,000) is essentially unchanged when derogatory events are added---$0.00390\rightarrow0.00393$ for the LPM---confirming that the two information sources (income and credit history) are largely orthogonal.
\item \textbf{Demographics stay stable.} \texttt{selfempl} remains economically large ($\approx -10$ pp in the LPM) and significant in both specifications. \texttt{ownrent} retains a positive, $\approx 4.5$ pp effect.
\item \textbf{Age flips to insignificance.} In the reduced model $\hat\beta_{\text{age}} = -0.00130$ ($p=0.003$); once derogatory events are controlled, age becomes insignificant ($p=0.55$ for the LPM, $p\approx 0.70$ for probit/logit). The apparent ``age effect'' in (b)--(c) was largely a proxy for accumulated derogatory marks.
\item \textbf{Address tenure becomes significant.} \texttt{acadmos} gains statistical significance in the full model ($p<0.01$) with a very small coefficient, indicating a modest but real stability-of-residence signal once credit history is held fixed.
\end{itemize}
\end{solution}
...
\end{problem}
\begin{solution}
\textbf{Best-fitting model.} Among the three with derogatory-event controls, the \emph{logit} narrowly wins on every fit criterion: log-likelihood $-5066.37$ (vs.\ $-5080.81$ for probit and the LPM's OLS $R^2 = 0.2118$ with RMSE $0.366$), and pseudo-$R^2$ $0.2318$ (vs.\ $0.2296$ for probit). All three models agree that introducing \texttt{majordrg} and \texttt{minordrg} multiplies the fraction of variance in \texttt{cardhldr} that we can explain by roughly $8$. The probit and logit full-model fits are essentially tied---a common finding when the link functions are close to linear on the bulk of the sample.

\textbf{Most explanatory variable.} Several metrics point to \texttt{majordrg}:
\begin{enumerate}
\item Its standardized coefficient ($\hat\beta \cdot \mathrm{sd}(X) = -0.117 \times 1.44 \approx -0.17$ in the LPM) is the largest in absolute terms, roughly twice that of \texttt{selfempl} or \texttt{ownrent}.
\item Its $t$-statistic is the most extreme ($|t| \approx 27$ in the LPM, $|z| \approx 33$ in probit/logit).
\item Its inclusion is what drives the jump in fit between columns (1)--(3) and (4)--(6); a re-run excluding \texttt{majordrg} but retaining \texttt{minordrg} recovers only a small share of that gain.
\end{enumerate}

\textbf{Do major derogatory events matter?} Unambiguously yes. Each additional event reduces approval probability by roughly $11$--$17$ pp depending on the evaluation point (see part~(j) for the median-$X$ marginal effects). This is the largest economic effect of any variable in the specification and the effect that drives almost all of the explanatory power gained relative to the reduced model.

\textbf{Preference.} I prefer the model with derogatory events. The reduced specification suffers from severe omitted-variable bias: \texttt{selfempl} and \texttt{age} coefficients shrink (in the case of age, toward zero), and the $R^2$ is an order of magnitude smaller. More substantively, any credible account of the credit-approval process must include prior credit behavior, and the data bear this out overwhelmingly.
\end{solution}
...
\textbf{Questions for the TA.}
\begin{enumerate}
\item[(1)] At the median, \texttt{majordrg} $= 0$---the distribution is highly skewed, and the median is not a ``typical'' applicant with adverse events. Would it be more informative to evaluate the marginal effect at a representative nonzero point (e.g., the mean, $\approx 0.47$), or to report the APE?
\item[(2)] The probit/logit delta-method SE conditions on the point $X_0$ at which we evaluate the derivative. Does that understate the sampling variability that a re-interpretation (``what if the median were different?'') would pick up? Would a bootstrapped SE be materially different here?
\item[(3)] For a count-valued regressor like \texttt{majordrg}, is it more natural to report the \emph{discrete} change $P(\text{majordrg}=1) - P(\text{majordrg}=0)$ instead of the derivative? How does that quantity compare to the $-0.172$ reported above?
\end{enumerate}
\end{reflection}
...
\end{problem}
\begin{solution}
Because $Y$ is a probability, the legitimate domain of any fitted value is $[0,1]$. The LPM does not respect this constraint: its fitted index $\hat P_i = X_i'\hat\beta$ is a linear function of the covariates and will take arbitrarily extreme values when some regressor is far from its mean. In this sample,
\begin{itemize}
\item $\amexLPMpctbelow$ of observations have $\hat P^{\text{LPM}} < 0$,
\item $\amexLPMpctabove$ of observations have $\hat P^{\text{LPM}} > 1$,
\item $\amexLPMpctout$ in total lie outside $[0,1]$, with fitted values ranging from $\amexLPMmin$ to $\amexLPMmax$.
\end{itemize}
These out-of-range predictions are \emph{not} interpretable as probabilities; a value of $-1.8$ means nothing more than ``the linear predictor is highly negative,'' which in turn is a signal that the applicant has so many derogatory events or so low an income that the true probability is essentially zero. In practice we either (i) truncate the prediction at $0$ or $1$ (standard defensive move but biased in finite samples), (ii) switch to a nonlinear model whose link function guarantees $\hat P \in (0,1)$ (the probit or logit solution), or (iii) fit a \emph{fractional} logit \citep[see][]{AngristPischke2009} which combines the LPM's computational simplicity with the logistic CDF's bounded range. The fact that $\approx 4\%$ of the sample produces nonsensical LPM predictions is one of the standard textbook arguments for probit/logit when the outcome is highly unbalanced or the regressors are dispersed.
\end{solution}
...
\end{problem}
\begin{solution}
\textbf{Probit vs.\ logit.} Despite using different link functions, the two nonlinear models deliver almost identical conclusions:
\begin{enumerate}
\item The \emph{signs} of every coefficient agree.
\item The \emph{statistical significance} of every coefficient agrees (both models flag \texttt{minordrg} as marginally significant at the $10\%$ level and every other regressor at the $1\%$ level).
\item The \emph{implied marginal effects at the median} are within $\sim 0.006$ of one another for \texttt{majordrg} (part~(j)) and within $\sim 0.002$ for the dummies (part~(c)).
\item The log-likelihoods and pseudo-$R^2$ are essentially tied ($-5066.4$ vs.\ $-5080.8$; $0.232$ vs.\ $0.230$).
\end{enumerate}
The logit coefficients are roughly $\sqrt{\pi^2/3} / 1 \approx 1.81$ times the probit coefficients---the well-known rescaling owing to different error variances in the latent-variable formulations---but the implied probabilities are nearly identical once that scaling is absorbed by the link function.

\textbf{LPM.} The LPM gets the sign and significance pattern exactly right and delivers coefficients whose magnitudes are easy to read directly as marginal effects. It fails in two respects documented above: (1) $\approx 4\%$ of fitted values exit $[0,1]$ (part~(l)), and (2) the coefficient is a \emph{constant} marginal effect and so cannot reproduce the subtle dependence of the nonlinear models' marginal effects on the baseline $X$ (part~(d)). In this sample these failures are empirically minor: if the goal is to report average partial effects, the LPM and the probit/logit APEs differ by pennies. If the goal is accurate prediction of individual probabilities---especially for applicants with high \texttt{majordrg} or low \texttt{inc}---the LPM is inadequate and probit/logit should be preferred.
\end{solution}
...
\[
\frac{\hat\beta^{\mathrm{LPM}}}{\hat\beta^{\mathrm{probit}}} = \amexRatioPROB \;\; (\text{rule-of-thumb } 0.40),
\qquad
\frac{\hat\beta^{\mathrm{LPM}}}{\hat\beta^{\mathrm{logit}}} = \amexRatioLOG \;\; (\text{rule-of-thumb } 0.25).
\]
\textbf{Why the rule fails here.} The \citet{Amemiya1981} approximation depends on evaluating the link-function derivative over the range $P \in [0.3, 0.7]$, precisely the interior where $\phi(z)$ and $\Lambda'(z)$ are largest. But \texttt{majordrg} has a long right tail ($\max = 22$), and applicants with several derogatory events face approval probabilities near zero---far outside the interior band. The LPM's coefficient is an average slope over the full covariate support, while the probit/logit coefficient captures the latent-index slope where the CDF is near-vertical on either tail. The two are not comparable when the regressor drives probabilities into the extremes.

This is a substantive caution: the $0.4/0.25$ rule is a rough internal-consistency check, not a law. When a regressor generates large probability variation---precisely when we care most about the coefficient---the approximation breaks down. See \citet[Ch.~3]{AngristPischke2009} for the general point that the LPM coefficient is better compared to the \emph{average} partial effect from probit/logit, not to the rescaled index coefficient.
\end{solution}
...
\textbf{Questions for the TA.}
\begin{enumerate}
\item[(1)] \citet{Amemiya1981} derives the $0.4$ and $0.25$ constants by linearizing the CDF around $P = 0.5$; in our data the unconditional $\hat p = 0.78$ is already outside that interior. Is there a known extension of the rule that corrects for the baseline probability, analogous to the ``nonlinear-link correction'' in \citet{CameronTrivedi2005}?
\item[(2)] Should we report the LPM/APE ratio rather than the LPM/index-coefficient ratio? At the sample APE, $\partial P / \partial\texttt{majordrg}^{\mathrm{probit}} \approx \hat\beta^{\mathrm{probit}} \cdot \bar{\phi}$ where $\bar\phi$ is the average link-function derivative. Empirically this average is $\approx 0.165$ in our sample, which gives a predicted LPM of $-0.706 \times 0.165 = -0.116$---within $1\%$ of the actual LPM coefficient.
\item[(3)] If we intended to use the Amemiya rule for model selection (e.g., ``accept the LPM if the ratio is close to $0.4$''), what is the appropriate tolerance? Is a ratio of $0.30$ close enough? Or does any ratio below, say, $0.35$ suggest that the LPM is picking up a different feature of the distribution than the nonlinear model?
\end{enumerate}
\end{reflection}
...
The \texttt{do\_files/} subfolder contains three heavily annotated Stata do-files: \texttt{ps6\_amex.do} (solving Problem~1, parts a--n), \texttt{ps6\_problem1.do} (solving SW~11.1), and \texttt{ps6\_problem2.do} (solving SW~11.2). Each of these writes to a matching \texttt{.log} in the same directory. The \texttt{output/} subfolder contains the \LaTeX\ tables (\texttt{output/tables/}) and academic-quality PDF figures (\texttt{output/figures/}) generated by those do-files and \texttt{\textbackslash input}ed into this document.
